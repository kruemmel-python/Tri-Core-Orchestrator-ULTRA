...
- Parametrische LR-Kopplung: exp / sigmoid / tanh / linear / mix (UI-Parameter)
...
- Stabil: Bibliothekspfad robust (ENV > Query > OS-Default), GPU-Index in Query-Param
...
# =============================================================================
# OS-Defaults & Loader-Helfer (Linux .so als Default)
# =============================================================================
...
def _default_lib_candidates() -> list[Path]:
    cands: list[Path] = []
    if IS_WIN:
        cands += [Path("./build/CipherCore_OpenCl.dll"), Path("CipherCore_OpenCl.dll")]
    elif IS_MAC:
        cands += [Path("./build/libCC_OpenCl.dylib"), Path("libCC_OpenCl.dylib")]
    else:  # Linux
        cands += [Path("./build/libCC_OpenCl.so"), Path("libCC_OpenCl.so")]
    # Basename-Fallbacks (LD_LIBRARY_PATH / dl-cache)
    cands += [Path("CipherCore_OpenCl.dll"), Path("libCC_OpenCl.so"), Path("libCC_OpenCl.dylib")]
    return cands

def _resolve_dll_path_from_env_or_query(query_val: Optional[str]) -> Path:
    env = os.getenv("CIPHERCORE_DLL", "").strip()
    if env:
        return Path(env)
    if query_val:
        p = Path(query_val)
        if IS_LINUX and (":" in query_val or "\\" in query_val):
            pass  # Windows-Pfade auf Linux ignorieren
        else:
            return p
    for cand in _default_lib_candidates():
        if cand.exists():
            return cand
    return _default_lib_candidates()[0]
...
def load_dll(dll_path: Path) -> ct.CDLL:
    # RTLD_GLOBAL sichert Symbol-Resolution für nachgeladene Libs
    rtld_global = getattr(ct, "RTLD_GLOBAL", 0)
    try_candidates: list[str] = []

    # 1) Vollpfad
    try_candidates.append(str(dll_path.resolve()))

    # 2) Basename, falls Datei nicht existiert (LD_LIBRARY_PATH)
    if not dll_path.exists():
        try_candidates.append(dll_path.name)

    last_exc: Exception | None = None
    for cand in try_candidates:
        try:
            dll = ct.CDLL(cand, mode=rtld_global) if rtld_global else ct.CDLL(cand)
            break
        except Exception as e:
            last_exc = e
    else:
        raise RuntimeError(f"Bibliothek konnte nicht geladen werden. Versucht: {try_candidates}. "
...
def parse_pauli_z_terms(json_text: str) -> list[tuple[int, float]]:
    ...
    for i, item in enumerate(data):
        ...
        z = int(item["z_mask"]); c = float(item["c"])
        if z < 0:
...
def run_pipeline_once(ctx: DriverCtx,
    ...
    # Per-Proto-Metriken
    d_emb = np.linalg.norm(prototypes_updated - proto_before, axis=1)
    d_pca = np.linalg.norm(proj_after - proj_before, axis=1)
...
with st.sidebar:
    ...
    # --- Bibliothekspfad: ENV > Query > Default-Kandidaten ---
    if "dll_path" not in st.session_state:
        qp_dll = st.query_params.get("dll", [None])[0]
        st.session_state["dll_path"] = str(_resolve_dll_path_from_env_or_query(qp_dll))

    dll_path_str = st.text_input(
        "Treiber-Bibliothek (DLL/.so/.dylib)",
        value=st.session_state["dll_path"],
        help=("ENV CIPHERCORE_DLL wird vorrangig genutzt. "
              "Unter Linux typischerweise ./build/libCC_OpenCl.so"),
    )
    ...
    try:
        ...
        dll_loaded = True
        st.caption(f"✅ Bibliothek geladen: {dll_path.resolve() if dll_path.exists() else dll_path}")
        if IS_LINUX:
            st.caption("💡 Linux: Falls die .so nicht gefunden wird, setze LD_LIBRARY_PATH oder lege sie in ./build/")
    except Exception as e:
        st.error(f"Bibliothek konnte nicht geladen werden: {e}")
    ...
    st.query_params["gpu"] = str(st.session_state["gpu_index"])  # Nur GPU in URL, nicht die DLL!
    ...
    hjson = st.text_area("Terme (Liste von Objekten mit z_mask und c)",
    ...
    with col2:
        ...
        if up is not None and st.button("🔁 Laden"):
            try:
                ...
                st.session_state["proto_mask_strength"] = float(params.get("proto_mask_strength", proto_mask_strength))
                st.session_state["vqe_shots"] = int(params.get("vqe_shots", vqe_shots))
                st.session_state["ansatz"] = params.get("ansatz", ansatz)
                st.session_state["gate_set"] = params.get("gate_set", gate_set)
                st.session_state["optim_method"] = params.get("optim_method", optim_method)
                st.session_state["use_analytic_grad"] = params.get("use_analytic_grad", use_analytic_grad)
...
# -----------------------------------------------------------------------------
# PCA: Interaktiv + Zeitverlauf + GIF-Export
# -----------------------------------------------------------------------------
ed = st.session_state["epoch_data"]
